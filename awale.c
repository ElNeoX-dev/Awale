...
int isFinish(Client **clients, int *plateau, int *points, int *authorizedMove, int *joueur)
{
    ...
    if ((sumJ1 == 0 || sumJ2 == 0) && (checkStarvation(plateau, authorizedMove, 0) == 0) && (checkStarvation(plateau, authorizedMove, 1) == 0))
    {
        ...
        if (points[0] > points[1])
        {
            write_to_players(clients, "%s wins!\n", joueur[0]);
        }
        else if (points[1] > points[0])
        {
            write_to_players(clients, "%s wins!\n", joueur[1]);
        ...
        {
            write_to_players(clients, "It's a tie!\n");
        }
...
void jouer(Client client, int *plateau, char **joueur, int *authorizedMove, int *points, int j)
{
    ...
    if (updateAuthorizedMove(authorizedMove, plateau, j) == 1)
    {
        write_client(client->sock, "\e[0;31m\033[1mStarvation !\033[0m\r\n");
        write_client(client->sock, "\e[0;32m\033[1mCases possibles : ");
        for (i = 0; i < 6; i++)
        {
            if (authorizedMove[i] == 1)
            {
                write_client(client->sock, "%d ", i + j * 6);
            }
        }
    ...
    while (validMove != 1)
    {
        if (j == 0)
        {
            write_client(client->sock, "%s, choisissez une case non-vide (entre 0 et 5): ", joueur[j]);
            scanf("%d", &caseChoisie);
            write_client(client->sock, "\r\n");
        }
        else
        {
            write_client(client->sock, "%s, choisissez une case non-vide (entre 6 et 11): ", joueur[j]);
            scanf("%d", &caseChoisie);
            write_client(client->sock, "\r\n");
        }
        ...
        if (caseChoisie < j * 6 + 6 && caseChoisie >= j * 6)
        {
            if (authorizedMove[caseChoisie - j * 6] == 0)
            {
                write_client(client->sock, "\e[0;31m\033[1m/!\\ Choix non valide !\033[0m\r\n");
                validMove = 0;
            }
            else
            {
    ...
    while (isTerrainAdverse(j, caseChoisie) && plateau[caseChoisie] > 1 && plateau[caseChoisie] < 4)
    {
        write_client(client->sock, "\033[1m%s prend les graines de la case %d\033[0m\r\n", joueur[j], caseChoisie);
        points[j] += plateau[caseChoisie];
        plateau[caseChoisie] = 0;
    ...
    if (checkStarvation(plateau, authorizedMove, j))
    {
        write_client(client->sock, "\033[1m%s a affamé son adversaire ! Il ne prend donc pas les graines...\033[0m\r\n", joueur[j]);
        for (int i = 0; i < 12; i++)
        {
            plateau[i] = plateauSave[i];
        }
